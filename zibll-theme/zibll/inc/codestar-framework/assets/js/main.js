    ...
    $.fn.csf_page_templates = function () {
        if (this.length) {
            $(document).on('change', '.editor-page-attributes__template select, #page_template', function () {
                ...
                $('.csf-page-' + maybe_value.toLowerCase().replace(/[^a-zA-Z0-9]+/g, '-'))
                ...
                CSF.helper.debounce(function () {
                    ...
                    if (value.length > 1) {
                        ...
                        $fields.each(function () {
                            ...
                            var text = '';
                            $(this)
                                .find('.csf-title,.csf-fieldset>.csf--label,.csf-fieldset>.csf-desc-text, .csf-search-tags')
                                .each(function () {
                                    text += $(this).text();
                                });
            ...
            base.notificationOverlay = function () {
                if (wp.customize.notifications && wp.customize.OverlayNotification) {
                    ...
                    wp.customize.notifications.add(
                        new wp.customize.OverlayNotification('csf_field_backup_notification', {
                            type: 'default',
                            message: '&nbsp;',
                            loading: true,
                        })
            ...
            $this.on('click', function (e) {
                if (!flooding) {
                    ...
                    if ($this.hasClass('csf-save-ajax')) {
                        ...
                        window.wp.ajax
                            .post('csf_' + $panel.data('unique') + '_ajax_save', {
                            ...
                            .fail(function (response) {
                                console.error('保存时发生错误，请参看此教程进行处理：https://www.zibll.com/forum-post/17398.html');
            ...
            $input.wpColorPicker({
                ...
                create: function () {
                    ...
                    var a8cIris = $input.data('a8cIris'),
                        $transparent_wrap = $('<div class="csf--transparent-wrap">' + '<div class="csf--transparent-slider"></div>' + '<div class="csf--transparent-offset"></div>' + '<div class="csf--transparent-text"></div>' + '<div class="csf--transparent-button">transparent <i class="fas fa-toggle-off"></i></div>' + '</div>').appendTo($container.find('.wp-picker-holder')),
